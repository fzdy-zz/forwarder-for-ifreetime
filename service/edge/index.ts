...
export class Service {
  ...
  private timer: NodeJS.Timeout | null = null; // 修改为 NodeJS.Timeout
  ...
  private async connect(): Promise<WebSocket> {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('close', (code, reason) => {
        this.ws = null;
        if (this.timer) {
          clearTimeout(this.timer); // 使用 NodeJS.Timeout 类型
          this.timer = null;
        }
      ...
      ws.on('message', (message, isBinary) => {
        let pattern = /X-RequestId:([a-z0-9]*)/; // 改为普通捕获组
        if (!isBinary) {
          console.debug('收到文本消息：%s', message);
          let data = message.toString();
          if (data.includes('Path:turn.start')) {
            let matches = data.match(pattern);
            let requestId = matches ? matches[1] : '';
